...
_CONFIG_PATH = str(Path(__file__).resolve().parents[2] / "research" / "config")
...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.validate_only:
        logger.info("Validated ablation configuration for case={}", case_name)
        return
    ...
    if final_state is None:
        raise RuntimeError(
            "Pipeline finished without producing a final state — check the logs for the failure."
        )
...
def _load_cases(cases_path: Path) -> dict[str, dict[str, str]]:
    ...
    return {p.stem: yaml.safe_load(p.read_text(encoding="utf-8")) for p in cases_path.glob("*.yaml")}
