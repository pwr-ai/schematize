...
# Usage: bash research/scripts/schematiq/setup_schematiq.sh [ref]
set -euo pipefail
...
cd "$REPO_ROOT"
uv sync --extra huggingface --extra research
...
Next steps:
  API_KEY=<litellm-key> API_URL=http://localhost:4000 bash research/scripts/schematiq/baseline_schematiq.sh
  API_KEY=<litellm-key> API_URL=http://localhost:4000 bash research/scripts/schematiq/eval_schematiq.sh
EOF
