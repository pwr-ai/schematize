...
``research/scripts/evaluate_schema.py`` can score it unchanged.

Usage:
    uv run python research/scripts/schematiq/run_schematiq_baseline.py --case pl_age --model gpt-5.4-mini
"""
...
_DEFAULT_CASES = REPO_ROOT / "research" / "data" / "cases"
...
def main(
    ...
    validate_only: Annotated[bool, typer.Option("--validate-only", help="Validate inputs without running ScheMatiQ")] = False,
) -> None:
    case_data = _load_case(cases_path, case)
    if validate_only:
        typer.echo(f"Validated ScheMatiQ baseline inputs for case={case}")
        return
